...
export default function WaitingForCompany() {
  ...
  const handleWaitingText = (status, requestedCompanyName) => {
    switch (status) {
      case 'rejected':
  ...
  async function getRequestData() {
    ...
    handleWaitingText(status, requestedCompanyName)
  }
  ...
  const handleRefresh = () => {
    getCompanyID(pmId).then(async (data) => {
      if (data[0]){
        const updatedUser = structuredClone(currentUser)
        updatedUser.company_id = data[0].company_id
